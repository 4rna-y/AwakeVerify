...
deployment_name="${AZURE_DEPLOYMENT_NAME:-awaver-nonprod-$(date +%Y%m%d%H%M%S)}"
...
printf 'Deploy foundation resources (without Backend/Worker workloads) as %s to resource group %s. Continue? [y/N] ' "$deployment_name" "$resource_group"
read -r answer
