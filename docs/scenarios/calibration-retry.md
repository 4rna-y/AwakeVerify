...
12. Workerがサービス資格情報で成功結果と `sourceSequenceNo` をBackendへ送る。Backendは成功した1件だけを `calibrations` に冪等保存し、同一トランザクションで `calibration_status: succeeded` の通知Outboxを登録する。失敗試行はDB保存しない。
13. Outboxからの成功通知を受信後、フロントエンドが動画教材の再生を許可する。
...
- 通知が一時的に失敗しても成功結果は永続化され、再試行配信される。
