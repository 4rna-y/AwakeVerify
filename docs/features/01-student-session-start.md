...
1. 学籍番号入力フォームを表示する。
2. 受講者が学籍番号を入力する。
3. `POST /api/sessions` を呼び出す。
4. レスポンスの `sessionId` を保持する。
5. 取得した `sessionId` を以下に使用する。
   - WebSocket接続
   - SignalR購読
   - 停止・再開イベント送信
   - 受講中状態管理
...
| 状態 | 条件 | 表示 | 操作 |
| --- | --- | --- | --- |
| `idle` | 初期表示 | 学籍番号入力フォーム | セッション開始ボタンを表示する。 |
| `starting` | セッション作成中 | 読み込み表示 | セッション開始ボタンを無効化する。 |
