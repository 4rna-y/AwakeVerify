...
BackendとWorkerの起動確認に成功した場合のみ、キャリブレーションを開始する。
...
- `GET /api/sessions/{sessionId}/analysis-events` のイベントストリームを購読する。
- WorkerがBackendへpublishした `drowsiness_score` / `tracking_status` を表示する。
