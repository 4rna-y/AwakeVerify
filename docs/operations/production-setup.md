...
非本番環境のBicep、secure parameter契約、`az` によるvalidate/deploy、warm-up、scale-to-zero、監視項目は [`../../infra/azure/README.md`](../../infra/azure/README.md) を正とする。対象subscription、リージョン、quota、コスト上限が明確でない限り、実デプロイを行わない。
...
| 対象 | 監視内容 |
| --- | --- |
| Backend | 5xx、401/403急増、応答時間、Outbox未配信件数・最古Outbox年齢・claim/lease期限切れ、migration失敗 |
...
ログには学生ID、password、Cookie値、Bearer token、API key、接続文字列を出力しない。
