...
| `docs/features/09-realtime-notification.md` | 通知payloadに含める解析結果の生成までを検証する。実際のSignalR送信は本番Worker側で扱う。 |
...
6. 本番 `src/worker/app` から `shared` を呼び出す形でService Bus / Blob / DB / SignalRへ接続する。
...
- Blobからフレームを取得できる
- shared解析ロジックを呼び出せる
- キャリブレーション結果をPostgreSQLへ保存できる
- 眠気スコアをPostgreSQLへ保存できる
- SignalR通知payloadを生成・送信できる
...
モデルファイルはWorker専用の実行時アセットとして `src/worker` 配下に置く。Git管理するか、READMEでダウンロード手順を示すかは別途決定する。
...
- 本番での保存・通知責務は `src/worker/app` に残す。
